(function($) {
  ...
  Drupal.futurehistoryEntdecken.setMapArrow = function(mapId, marker, i) {
  ...
  Drupal.futurehistoryEntdecken.DateSlider = function (mapId,date){
    ...
    $("#time_slider").on( "slidestop", function( event, ui ) {
      Drupal.futurehistoryEntdecken.getMarkers(bounds, date, kategorie, sort, mapId, mapCenter);
    });
  };
  ...
  Drupal.futurehistoryEntdecken.getMarkers = function (bounds, date, kategorie, sort, mapId, mapCenter){
    ...
    $.ajax({
      url: poi_url ,
      method: 'get',
      data: {bbox : bbox, date : date, field_kategorie_name : kategorie.toString() }, 
      dataType: 'json',   
    ...
    $('#thumbnail-pois').empty();
      ...
      for ( var i = 0; i< marker_content.length; i++) {
        dist = google.maps.geometry.spherical.computeDistanceBetween(
          new google.maps.LatLng(marker_content[i].lat, marker_content[i].lon), mapCenter);
      ...
      for (var i in marker_content) {
        ...
        $('#thumbnail-pois').append(poi_element);
         ...
         if (last_active_nid) {
           // no movement in loop, take just the last on top
           //$('#thumbnail-pois').scrollTo('li#'+last_active_nid+'', 1000, {offset:20});
           // try to find the first in list (if there are double points....)
           $('#thumbnail-pois').scrollTo('li.active', 1000, {offset:20});
         }
       }
  ...
  Drupal.futurehistoryEntdecken.setMapMarkers = function(marker_content, mapId) {
    ...
    $.each(marker_content, function() {
      ...
      for ( var n = 0; n < last_active_NIDs.length; n++) {
        ...
        if (marker.id == last_active_NIDs[n]) {
          marker.setIcon(fh_marker_violet);
          $('#thumbnail-pois li#'+marker.id+'').addClass('active');
          marker.setZIndex(google.maps.Marker.MAX_ZINDEX + 1);
          Drupal.futurehistoryEntdecken.setMapArrow(mapId, marker, 0);
          //console.log('setMarker Arrow');
        }
      }
  ...
  Drupal.futurehistoryEntdecken.markerHighlight = function(markerId, action, mapId) {
    ...
    // reset all selected markers / humbs
    ...
    for ( var n = 0; n < last_active_NIDs.length; n++) {
      // deactivate the former activated thumbnails
      $('#thumbnail-pois li#'+last_active_NIDs[n]+'').removeClass('active');
      $('#tc-'+last_active_NIDs[n]+'').hide();
    }
    last_active_NIDs = [];
    ...
    for ( var i = 0; i< Drupal.futurehistoryEntdecken[mapId].markers.length; i++) {
        ...
        if (action == "hover") {
         ...
         if (action == "click") {
           if ( Drupal.futurehistoryEntdecken[mapId].markers[i].id == last_active_nid ) {
             last_active_nid = 0;
             Drupal.futurehistoryEntdecken.delMapArrow(0);
           }
           else {
             // highlight me
      ...
      // activate all markers identified (double check result in last_active_NIDs-Array)
      for ( var n = 0; n < last_active_NIDs.length; n++) {
        //toggle the thumbnail control
        if (Drupal.futurehistoryEntdecken[mapId].markers[i].id == last_active_NIDs[n]) {
          //console.log(' act ', last_active_NIDs[n]);
          Drupal.futurehistoryEntdecken[mapId].markers[i].setIcon(fh_marker_violet);
          Drupal.futurehistoryEntdecken[mapId].markers[i].setZIndex(google.maps.Marker.MAX_ZINDEX + 1);
          Drupal.futurehistoryEntdecken.setMapArrow(mapId, Drupal.futurehistoryEntdecken[mapId].markers[i], 0);
          //console.log('MarkerHighlite Arrow');
          $('#thumbnail-pois li#'+last_active_NIDs[n]+'').addClass('active');       
          $('#tc-'+last_active_NIDs[n]+'').slideDown("slow");
          $('#thumbnail-pois').scrollTo('li#'+last_active_NIDs[n]+'', 100, {offset:-200});
        }
      }
  ...
  Drupal.behaviors.futurehistoryEntdecken = {
    attach: function (context, settings) {
      ...
      $('.futurehistory-entdecken-map', context).each(function() {
        ...
        //if (fh_cookiedata != null || fh_cookiedata.viewport == '0' || fh_cookiedata.viewport == '1'   ) {
