(function($) {
  ...
  var fh_marker_trans = new google.maps.MarkerImage(
    '/sites/default/files/gmap-files/trans.png',
    new google.maps.Size(25, 25),
    new google.maps.Point(0, 0), //origin
    new google.maps.Point(12, 12) //anchor point
  );
  ...
  Drupal.futurehistoryEntdecken.deactivateMarker = function(mapId, id) {
    for ( var k = 0; k< Drupal.futurehistoryEntdecken[mapId].markers.length; k++) {
      if (Drupal.futurehistoryEntdecken[mapId].markers[k].id === id){
        ...
        Drupal.futurehistoryEntdecken[mapId].markers[k].setIcon(fh_marker_blue);
  ...
  Drupal.futurehistoryEntdecken.setMapMarkers = function(marker_content, mapId) {
    ...
    if(Drupal.futurehistoryEntdecken[mapId].map.getZoom() < 18) {
      var markerCluster = new MarkerClusterer(
        ...
        {imagePath: '/sites/default/files/gmap-files/m'}
      );
      if (markerCluster.getTotalClusters() > 0) {
        console.log('NUM Clusters: ', markerCluster.getTotalClusters())
      }
       console.log (markerCluster.FHinfo);
      for ( var c = 0; c < markerCluster.markers_.length; c++) {
        if (markerCluster.markers_[c].isAdded) {
          console.log ('ID ', markerCluster.markers[c].id, ' ADDED');
        }
      }
    } else {
