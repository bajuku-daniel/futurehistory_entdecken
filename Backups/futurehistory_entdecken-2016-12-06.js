(function($) {
  ...
  Drupal.futurehistoryEntdecken.setMapMarkers = function(marker_content, mapId) {
          ...
          if (dist < double_swell) {
            ...
            if ( LAST_ACTIVE_NID > -1 ) {
              if  ( RAW[x].id == LAST_ACTIVE_NID ) {
                 ...
                 // console.log('parent icon active ', RAW[r].id);
    ...
    // set Icons / pie
    for ( var r = 0; r < RAW.length; r++) {
      if ( RAW[r].activated || RAW[r].hiddenactive ) {
        if ( RAW[r].hideother ) {
    ...
    if ( src == 'THUMB' ) {
      for ( var i = 0; i < RAW.length; i++) {
        if ( RAW[i].id == markerId) {
          // console.log('call markerStateChange on ', markerId, ' data ', RAW[i]);
          
          if ( RAW[i].id.activated == true) {
            RAW[i].activated = false;
  ...
  Drupal.behaviors.futurehistoryEntdecken = {
    attach: function (context, settings) {
      ...
      $('.futurehistory-entdecken-map', context).each(function() {
        ...
        mapZoom = Drupal.futurehistoryEntdecken[mapId].map.getZoom();
        // console.log(' Z ', Drupal.futurehistoryEntdecken[mapId].map.getZoom());
        ...
        google.maps.event.addListener(Drupal.futurehistoryEntdecken[mapId].map, "rightclick", function(event) {
          ...
          var centerControl = new CenterControl(centerControlDiv, Drupal.futurehistoryEntdecken[mapId].map, 
              baseUrl + encodeURI('#' + coords.lat() + '#' + coords.lng() + '#' + Drupal.futurehistoryEntdecken[mapId].map.getZoom()));
