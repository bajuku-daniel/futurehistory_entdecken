(function($) {
  ...
  Drupal.futurehistoryEntdecken.setMapMarkers = function(marker_content, mapId) {
    ...
    // Reset TimeSlider to marker-Timerange in BBOX values
    InitYearRange = [9999,0];
    ...
    $.each(marker_content, function() {
      ...
      if (parseInt(this.Jahr) < parseInt(InitYearRange[0])) {
        InitYearRange[0] = parseInt(this.Jahr);
      } 
      if (parseInt(this.Jahr) > parseInt(InitYearRange[1])) {
        InitYearRange[1] = parseInt(this.Jahr);
      }
      ...
      // only one marker or same Date? Spread range for slider Display
      if (InitYearRange[1] - InitYearRange[0] < 10) {
        InitYearRange[0] = parseInt(InitYearRange[0]) - 5;
        InitYearRange[1] = parseInt(InitYearRange[1]) + 5;
      }
